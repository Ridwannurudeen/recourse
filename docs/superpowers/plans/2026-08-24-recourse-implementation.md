...
- Batch proofs: use the SDK's `getBatchProof(hashes)`. It **does** support non-contiguous blocks and is the production path.
- **`mergeProofs` — the real rule, measured.** It does not require *adjacent* blocks; it requires each proof's covered range to reach the next proof's start block, and throws `Proofs are not contiguous` otherwise. A proof at height `H` covers `[H, H + roots.length - 1]`. Every proof from this service extends up to a **common upper checkpoint**, so `roots ≈ checkpoint - H + 1`. Consequences:

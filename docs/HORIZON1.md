...
`PolicyRegistryV1` is deployed separately in the qualified current V3 core at [`0xFb5B0d26140A8577015388E206A06fc6e1622c28`](https://creditcoin-testnet.blockscout.com/address/0xFb5B0d26140A8577015388E206A06fc6e1622c28), as recorded in [`deployments-v3-current.json`](../deployments-v3-current.json). [`activation-v3-current.json`](../activation-v3-current.json) records release `0xad31a01779b7c8c8651e1fecbb15b6d177c25dbd637c99d7496c2c2a0b7d221a` for the activated capped V3 testnet demonstration facility; this registry is not part of the live Horizon 1 facility path. The historical inactive [`deployments-v3.json`](../deployments-v3.json) remains preserved: that older core predates the current source-ordering interface and cannot be activated by the hardened tooling. A release binds an issuer namespace, issuer-declared build-artifact hash, reference runtime, declared evidence kinds, and metadata-only action-adapter specifications. The issuer can approve exact constructor-bound runtime variants and attest deployments whose facility, kernel, evaluator runtime, manifest, and configuration agree. Audit publishers attach their own identity to either an exact release snapshot or exact deployment snapshot; the registry never exposes a global "audited" flag.
...
The additive [public Horizon 1 console](https://recourse.gudman.xyz/horizon1.html), served from [`web/horizon1.html`](../web/horizon1.html), reads the live factory at one pinned block, filters facilities to the configured kernel and credit state, discovers registered policies from `PolicyRegistered` logs, and keeps an unapplied registration distinct from an accepted policy effect. It never requests a wallet or exposes a transaction path.

`daemon/job-discovery.mjs` is a signerless Proof Jobs catalog and metrics report. It scans only confirmed blocks, checks canonical block hashes, checkpoints cumulative metrics plus cached state and a bounded recent-event window in an atomic cursor, refreshes changed jobs in bounded batches, pins every hydrated job and policy read to the report block, and reports only observable coverage, completion, reveal, slash, release, and latency data. Schema v3 includes report generation time plus the pinned state block's number, hash, and timestamp. `--output` or `HORIZON1_DISCOVERY_OUTPUT_FILE` writes the last successful report atomically; a failed scan or serialization never replaces it. Partial history is marked incomplete and does not invent lifecycle denominators, invalid-proof rates, uptime, reputation, or economics.
...
It accepts only a successful Ethereum transaction that exactly matches the committed event policy, obtains the exact Attestcoin proof, commits a hunter-bound evidence digest and random salt, waits at least one CC3 block, and reveals. Before every approval, commit, or reveal broadcast it persists the proof, salt, commitment, signed raw transaction, and expected hash. A restart reconciles the receipt or mempool entry and can rebroadcast only that same signed transaction. The operator validates the deployment, job, evaluator, policy configuration, exact event qualifier, source receipt, proof cardinality, proof transaction hash, attested height, live commitment, and reveal deadline. Its gitignored state file is `daemon/horizon1-state.json`.

The long-running service remains read-only by default and is documented in [`PILOT-RUNBOOK.md`](PILOT-RUNBOOK.md). It adds explicit facility, policy, token, and source-chain allowlists; canonical per-job source cursors; transaction-bound execution journals; bounded retry; a single-instance lock; health output; and graceful signal handling.
...
The public static console is hosted at <https://recourse.gudman.xyz/horizon1.html>, alongside the other consoles at
<https://recourse.gudman.xyz>. For a local reproduction, serve the zero-build consoles from the repository root and open `/web/horizon1.html`:

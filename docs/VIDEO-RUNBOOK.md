...
## 1:45–2:35 — What is live today (Blockscout, the fresh V3 core + activated facility)
...
proven and adjudicated by anyone through the permissionless proof-job market."

## 2:35–3:05 — Honest limits (docs/ROADMAP.md, items 5–10 headings)
"What this is not: it is testnet software with an internal review, not an independent audit. Cross-chain remedies
wait on Attestcoin writability, which is not live. The portfolio pool is built but deliberately undeployed, because
its mandate requires a declared remedy adapter that no release can honestly declare yet. The SDK is published for
interface discovery, not as a frozen dependency."

...
Recourse has the live v1 and Horizon 1 applications on CC3 Testnet, plus a fresh hardened six-contract V3 core and an activated capped testnet facility. `deployments-v3-current.json` records the core as `deployed-qualified` from reviewed source commit `90d8b05af38940ffeb55d974401641a327176b9e`, with all six runtime code hashes verified. `activation-v3-current.json` records the activation; `deployments-v3.json` remains the historical inactive core. The proven v1 generation remains live and untouched: its five contracts provide three fixed covenant predicates, an ordered covenant-set commitment, a permissionless adjudicator, a native-token credit-facility state machine, the original autonomous hunter daemon, and a zero-build wallet application covering the lender, borrower, and hunter workflow. Facilities 1 and 2 both remain `Breached`.
...
A separate roadmap build adds the typed SDK and simulation package, `PolicyRegistryV1`, an issuer-declaration and exact-audit model, Proof Jobs discovery and observable-metrics reports, and walletless observatories. The committed Horizon 1 console is [publicly hosted](https://ridwan.gudman.xyz/recourse/horizon1.html); the SDK is published as `recourse-protocol-sdk@0.1.0` for interface discovery and testnet integration, and the current V3 registry has the pilot issuer release and deployment recorded in `activation-v3-current.json`. The historical V3 registry deployment contains no releases, deployments, or audit claims. It is part of the superseded core, not a current-ABI integration target. None of these foundations has been independently audited, frozen, or validated by an external integration.

The repository-buildable implementation for items 4–10 is now complete and catalogued in [Roadmap items 4–10: implemented scope and remaining gates](ROADMAP-4-10-BUILD.md). The inactive V3 deployment recorded in `deployments-v3.json` predates the hardened source-ordering interface and cannot be activated by the current tooling; it was superseded by the fresh full-core deployment in `deployments-v3-current.json`. A capped facility is activated using the fixed-supply demo token, with no design partner or live counterparty pilot. The USC route remains undeployed. The operator verifier and empty market are deployed and qualified on CC3 with one dedicated project-operated attestor; the fixed-vintage pool subsequently allocated 100,000 rUSD of project-operated test tokens to a newly created pool-owned facility backed by a 20,000 rUSD bond and issuer-attested policy deployment, followed by borrower activation, as recorded in `allocation-v3-portfolio-current.json` at block `5460383`. “Built” does not mean audited, integrated, funded, authorized, or validated by customers; the remaining external gates are preserved below.

Across the deployed generations and the local roadmap build, the suite passes 392 Forge tests, 314 root Node tests, and 44 SDK tests, followed by a strict SDK declaration compile; one Windows symlink test is skipped when the process lacks symlink privilege, for 315 root Node tests in total. Twelve stateful invariant checks (eight distinct properties) each complete 256 runs and 128,000 calls with zero handler reverts while checking native and ERC-20 asset conservation, claim solvency, facility and default-loss bounds, inactive credit availability, portfolio recovery and loss bounds, and fully collateralized operator-market obligations even when unsolicited token transfers create surplus. This remains testnet software. It has received internal adversarial review only, not an independent security audit; the demo asset is not a production stablecoin; and the demonstrated testnet history is not production credit performance.
...
### 4. A capped real-asset pilot — hardened core deployed; capped testnet facility activated

**What shipped:** `CappedPilotFactoryV1` fixes every counterparty and applies per-facility, aggregate-notional, bond, fee, maturity, delay, and count bounds; only its lender can consume capacity and its guardian can pause creation. It deploys `RecourseFacilityV3`, whose one-time lender-controlled default settlement applies the bond to remaining debt and returns only excess to the borrower. Keeping settlement lender-only makes a pooled lender's recovery delay authoritative. A fail-closed readiness evaluator, offline-by-default activation planner, read-only-by-default operator, recovery journal, service template, and runbook cover qualification without declaring the pilot production-ready. The historical six-contract V3 deployment is wired and has zero activation state, but its kernel and policy bytecode predate the current source-ordering contract and commitment, so the current activation tooling correctly rejects it. The fresh hardened core is deployed and runtime-qualified in `deployments-v3-current.json`; `activation-v3-current.json` records active facility `0x00B50626C4AA42d22ca01AAEa8649f253aEc5B1e`, policy `1`, registry release `0xad31a01779b7c8c8651e1fecbb15b6d177c25dbd637c99d7496c2c2a0b7d221a`, and open proof job `1`. The source window is Ethereum mainnet blocks `25944522` to `26024522`, facility maturity is CC3 block `5554121`, and proof-job expiry is Unix timestamp `1792497600`. The demonstration asset remains a fixed-supply testnet token, not a stablecoin or production asset.

**What did not ship:** No live pilot with a counterparty has been run; the activation demonstrates testnet mechanics only. Recourse still has no design partner, independent audit, legal review, production asset or custody decision, or production watcher. The historical scaffolding deployment must not be read as an activation target, evidence of customer demand, or production credit performance.
...
**Honest dependencies:** Unlike items 5 and 6, this item is not blocked on Attestcoin writability. The SDK is published as `recourse-protocol-sdk@0.1.0` for interface discovery and testnet integration, its interfaces are not frozen, no external protocol has integrated it, and no independent audit covers it. The current registry carries issuer declarations, not an audit verdict. At least two external integrations should shape the interfaces before they are frozen. Recourse has no external integration partner today. “Audited” must attach to an exact release or deployment and a named auditor, never to the registry as a whole.
...
### 9. Open operator market — verifier and empty market deployed; service activity not demonstrated

**What is implemented:** `OperatorMarketV1` supports separately priced monitoring, proof-construction, submission, and delivery quotes that are either public or restricted to one intended sponsor. The operator cannot self-sponsor. Acceptance binds the actual sponsor, service kind, terms, acceptance time, and service deadline; permissionless settlement passes that complete context plus the delivery and evidence to the configured verifier. Operator-bond and sponsor-price escrow, cancellation, expiry, and pull withdrawals keep every obligation fully collateralized even if unsolicited token transfers create surplus. The operator scanner checks confirmed canonical history, resumes through atomic reorg-checked cursors, pins hydration to one report block, and reports only event-observable metrics. Optional V3 execution is fail-closed on economics, exact allowlists, Kernel V2 stale-proof recovery, signer exclusivity, source-chain identity, canonical receipts, confirmation depth, fee ceilings, and durable transaction journals. The operator terminal refuses unanchored or inconsistent reports. A Horizon 1 instance is installed and healthy in read-only mode; no V3 executable service is installed or enabled. `deployments-v3-operator-service-verifier-current.json` and `deployments-v3-operator-market-current.json` record the verifier and empty market as deployed and qualified on CC3. The verifier uses one dedicated project-operated testnet EOA attestor, `0xeCf1BeeF05450f1E2A2adAb86b61ccC2D6235369`; qualification records zero quotes and zero market token balance. No operators, sponsors, settlements, completed work, or independent attestation have been demonstrated.
...
**Honest dependencies:** The quote and escrow contract is deployed but empty; no independently qualified production service verifier, active competitive operators, matching service, customers, reputation, or profitability evidence exists. Reverted invalid or irrelevant reveals emit no event, so the report does not claim a false-positive rate; partial history is explicitly incomplete. A real market still needs protection against censorship, job spam, collusion, and reward manipulation. Relaying should remain compatible with Attestcoin's own competitive fee market rather than becoming a separate Recourse network.

### 10. Programmable credit portfolios — project-funded testnet allocation and borrower activation demonstrated

**What shipped:** `PortfolioMandateV1` checks exact factory provenance, asset, kernel, facility status, notional, bond, fee, maturity, policy commitment, registry release/deployment binding, and evidence kind. Its action-adapter requirement is optional: zero requires no adapter declaration, while a nonzero kind retains exact matching of the registry declaration; neither mode proves remedy execution. `PortfolioPoolV1` is a fixed-vintage, single-asset capital pool with a frozen bounded investor allowlist, exact mandate revalidation, pool-created facilities, full-bond-before-allocation, one atomic full-limit allocation, pull-based claims, permissionless cash-only finalization, lender-only default-loss settlement after the pool's recovery delay, explicit loss recognition, delayed recovery redistribution, and bounded Proof Jobs service spending with an immutable 500-basis-point ceiling relative to maximum pool assets. It freezes each remedy evaluator/coordinator binding before funding and lets only the manager publish retries or request a policy-authorized replacement; those bounded servicing calls remain available after economic finalization so an open remedy is not stranded. Its largest-remainder accounting distributes each finalized or recovered asset unit exactly once. `deployments-v3-portfolio-core-current.json` records the pool, its dedicated pool-owned capped factory, and zero-mode mandate at initial deployment. No action adapter is required by this mandate. The subsequent `allocation-v3-portfolio-current.json` records pool `0x7dd538A9ab77a4d2953b28f3bCe710145a0eC8C2` allocating 100,000 rUSD to newly created pool-owned facility `0x0C874e56AD2dC9789A63a9Bc63c08a5F6D3C82C8`, backed by a 20,000 rUSD borrower bond and issuer-attested policy deployment. Policy 1 preserves the pilot configuration and required commitment. The borrower then activated the facility; both pool and facility were verified Active at block `5460383`. The 13 transactions cover creation, policy registration, issuer deployment record, candidate and investor registration, funding, bond posting, pool activation, allocation, and borrower activation. Investor and borrower are project wallets using project-operated test tokens: this is an allocation-path demonstration, not lender demand or external capital.
...
**Honest dependencies:** The pool is Active with a project-funded testnet allocation, remains unaudited, and is not an evergreen NAV product; shares lock after activation and only exact fixed-vintage allocations are supported. Meaningful live history, lender demand, legal structuring, custody review, valuation inputs, and audited loss accounting remain gates. It should follow successful bilateral facilities, not precede them.

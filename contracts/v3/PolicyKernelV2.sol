...
contract PolicyKernelV2 is ReentrancyGuard, IPolicyConfigurationContextV1, IProofJobsKernelV1 {
    ...
    mapping(address facility => mapping(uint256 policyId => mapping(bytes32 queryId => bool processed))) private
        processedQueries;
    mapping(address facility => mapping(uint256 policyId => mapping(uint64 chainKey => SourcePosition position)))
        private latestSourcePositions;
    ...
    function submitBatch(
        ...
        for (uint256 i; i < length; ++i) {
            ...
            if (
                previous.recorded && heights[i] < previous.blockHeight || previous.recorded
                    && heights[i] == previous.blockHeight && txIndex <= previous.transactionIndex
            ) revert StaleSourcePosition();
            previous.recorded = true;
            previous.blockHeight = heights[i];
            previous.transactionIndex = txIndex;
        ...
        if (
            result.sourceBlock != proven[length - 1].blockHeight
                || result.transactionIndex != proven[length - 1].txIndex
    ...
    function safeStaleProofRelease() external pure returns (bool) {
        return true;
    }

    function latestSourcePosition(address facility, uint256 policyId, uint64 chainKey)
        external
        view
        returns (bool recorded, uint64 blockHeight, uint64 transactionIndex)
    {
        SourcePosition storage position = latestSourcePositions[facility][policyId][chainKey];
    ...
    function _requireAdvancingSource(
        ...
        if (!_sourceAdvances(facility, policyId, chainKey, blockHeight, transactionIndex)) {
            revert StaleSourcePosition();
        }
    }

    function _sourceAdvances(
        address facility,
        uint256 policyId,
        uint64 chainKey,
        uint64 blockHeight,
        ...
        SourcePosition storage position = latestSourcePositions[facility][policyId][chainKey];
        return !position.recorded || blockHeight > position.blockHeight
